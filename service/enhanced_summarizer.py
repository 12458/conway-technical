...
def _build_enhanced_context(enriched_event: EnrichedEvent) -> str:
    ...
    context_parts.extend(
        [
            ...
            "",
            "- severity: Choose ONE of: low, medium, high, or critical. ",
            "  Always pick the HIGHEST level whose criteria are met. If none match, default to LOW.",
            "  Factors to weigh (in order): repo criticality/protection → actor trust/age → action type →",
            "  CI/security signals → commit entropy/size → popularity/consumption.\n",
            "  * CRITICAL: Destructive or confirmed-compromise actions on critical/protected assets:",
            "    - Force-push or branch/tag deletion on protected branches of critical repos",
            "    - Verified credential compromise, secret leakage, or confirmed malware backdoor",
            "    - Release/package tampering pushed to users, or entropy > 7.0 with corroborating indicators",
            "",
            "  * HIGH: Strongly suspicious activity with elevated blast radius:",
            "    - New/untrusted accounts (<7 days) performing sensitive actions on important repos",
            "    - Unsigned commits to PROTECTED branches or bypassed required checks",
            "    - Failed security checks (SAST/secret scan) tied to the change,",
            "    - Elevated entropy 6.0–7.0 PLUS another suspicious indicator (e.g., privilege change),",
            "    - Very large commits (>1000 LOC) from untrusted sources on important repos",
            "",
            "  * MEDIUM: Policy violations or unusual patterns with moderate risk:",
            "    - Unsigned commits to DEFAULT branch on repos of moderate/unknown criticality",
            "    - Minor workflow/CI failures without security findings",
            "    - Activity from inactive or recently reactivated accounts on standard repos",
            "    - Large commits (>500 LOC) without other red flags",
            "    - Spammy usernames/repo names in organization scope",
            "",
            "  * LOW: Benign or low-impact unusual activity, esp. with strong mitigating context:",
            "    - Maintenance/admin tasks by trusted/established maintainers",
            "    - Unsigned commit to DEFAULT branch on LOW-criticality repo by established actor,",
            "      small diff and normal entropy (<6.0), no corroborating anomalies",
            "    - Public but low-popularity repos with no indicators of downstream consumption",
            "    - No CI configured and no failing security checks, small scoped changes",
            "",
            "- severity_reasoning: 1–2 sentences explaining WHY you chose this severity",
            "  Reference specific enrichment data (actor age/trust, repo criticality/protection,",
            "  CI/security signals, commit entropy/size, popularity/consumers).",
            "",
            "- root_cause: 3–5 bullet points explaining what happened (use enrichment data)",
            "- impact: 3–5 bullet points on potential consequences (assess based on repo criticality, actor trust, consumers, etc.)",
            "- next_steps: 3–5 actionable remediation steps (prioritized by severity)",
            "- tags: Relevant security/incident classification tags (e.g., unsigned-commit, no-ci, default-branch, low-criticality, established-actor)",
        ]
    )
